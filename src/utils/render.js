...
function Renderer() {
  ...
  this.getTrackedTitles = function() {
    return _.cloneDeep(this._trackTitles.titles)
  }
  ...
  this.addHook = function(entryType, hookType, fnHook) {
    ;((this._hooks[entryType] = this._hooks[entryType] || {})[hookType] = this._hooks[entryType][hookType] || []).push(fnHook)
  }
  ...
  this._renderImage = function(entry, textStack, meta, options) {
    ...
    const svg =
      this._lazyImages && entry.width != null && entry.height != null
        ? `data:image/svg+xml,${encodeURIComponent(
            // eslint-disable-next-line max-len
            `<svg xmlns="http://www.w3.org/2000/svg" width="${entry.width}" height="${entry.height}"><rect width="100%" height="100%" fill="#ccc3"/></svg>`
          )}`
        : null
    textStack[0] += `<div class="${this._renderImage_getWrapperClasses(
      entry,
      meta
    )}"><a href="${href}" target="_blank" rel="noopener" ${
      entry.title ? `title="${Renderer.stripTags(entry.title)}"` : ''
    }><img class="rd__image" src="${svg || href}" ${entry.altText ? `alt="${entry.altText}"` : ''} ${
      svg ? `data-src="${href}"` : ''
    } ${getStylePart()}></a></div>`
    if (entry.title)
      textStack[0] += `<div class="rd__image-title"><div class="rd__image-title-inner">${this.render(entry.title)}</div></div>`
  ...
  this._renderImage_getUrl = function(entry) {
    ...
    if (entry.href.type === 'internal') {
      ...
      href = this.baseUrl !== '' ? `${this.baseUrl}${imgPart}` : imgPart
      ...
      for (let ixCell = 0; ixCell < len; ++ixCell) {
        ...
        if (roRender[ixCell].type === 'cell') {
          if (roRender[ixCell].roll) {
            ...
            } else if (roRender[ixCell].roll.max === Renderer.dice.POS_INFINITE) {
              toRenderCell = roRender[ixCell].roll.pad
                ? `${StrUtil.padNumber(roRender[ixCell].roll.min, 2, '0')}+`
                : `${roRender[ixCell].roll.min}+`
            } else {
              toRenderCell = roRender[ixCell].roll.pad
                ? `${StrUtil.padNumber(roRender[ixCell].roll.min, 2, '0')}-${StrUtil.padNumber(
                    roRender[ixCell].roll.max,
                    2,
                    '0'
                  )}`
        ...
        bodyStack[0] += `<td ${this._renderTable_makeTableTdClassText(entry, ixCell)} ${this._renderTable_getCellDataStr(
          roRender[ixCell]
  ...
  this._renderTable_getCellDataStr = function(ent) {
    ...
    if (ent.roll) {
      ...
      )}" data-roll-max="${convertZeros(ent.roll.exact != null ? ent.roll.exact : ent.roll.max)}"`
    }
  ...
  this._renderEntriesSubtypes = function(entry, textStack, meta, options, incDepth) {
    ...
    const pagePart =
      !isInlineTitle && entry.page > 0
        ? ` <span class="rd__title-link">${
            entry.source
              ? `<span class="help--subtle" title="${sourceJSONToFull(entry.source)}">${sourceJSONToAbv(entry.source)}</span> `
              : ''
          }p${entry.page}</span>`
        : ''
    ...
    const headerSpan = entry.name
      ? `<span class="rd__h ${headerClass}" data-title-index="${this._headerIndex++}" ${this._getEnumeratedTitleRel(
          entry.name
        )}> <span class="entry-title-inner"${
          !pagePart && entry.source
            ? ` title="Source: ${sourceJSONToFull(entry.source)}${entry.page ? `, p${entry.page}` : ''}"`
            : ''
        }>${this.render({ type: 'inline', entries: [entry.name] })}${isInlineTitle ? '.' : ''}</span>${pagePart}</span> `
      : ''
  ...
  this._renderEntriesSubtypes_getDataString = function(entry) {
    ...
    if (entry.type === 'optfeature' || entry.type === 'patron') {
      const titleString = entry.source ? `title="Source: ${sourceJSONToFull(entry.source)}"` : ''
      if (entry.subclass != null)
        dataString = `${ATB_DATA_SC}="${entry.subclass.name}" ${ATB_DATA_SRC}="${entry.subclass.source.source ||
          entry.subclass.source}" ${titleString}`
  ...
  this._renderEntriesSubtypes_getStyleString = function(entry, meta, isInlineTitle) {
    ...
    if ((entry.type === 'optfeature' || entry.type === 'patron') && entry.subclass != null)
      styleClasses.push(CLSS_SUBCLASS_FEATURE)
    return styleClasses.length > 0 ? `class="${styleClasses.join(' ')}"` : ''
  }

  this._renderOptions = function(entry, textStack, meta, options) {
    if (entry.entries) {
      entry.entries = entry.entries.sort((a, b) => (a.name && b.name ? sort(a.name, b.name) : a.name ? -1 : b.name ? 1 : 0))
  ...
  this._renderInset = function(entry, textStack, meta, options) {
    ...
    if (entry.name != null) {
      ...
      textStack[0] += `<span class="rd__h rd__h--2-inset" data-title-index="${this._headerIndex++}" ${this._getEnumeratedTitleRel(
        entry.name
  ...
  this._renderVariant = function(entry, textStack, meta, options) {
    ...
    textStack[0] += `<span class="rd__h rd__h--2-inset" data-title-index="${this._headerIndex++}" ${this._getEnumeratedTitleRel(
      entry.name
    )}><span class="entry-title-inner">Variant: ${entry.name}</span></span>`
    const len = entry.entries.length
    for (let i = 0; i < len; ++i) {
      const cacheDepth = meta.depth
      meta.depth = 2
  ...
  this._renderSpellcasting_getEntries = function(entry) {
    ...
    const toRender = [{ type: 'entries', name: entry.name, entries: entry.headerEntries ? _.cloneDeep(entry.headerEntries) : [] }]
    ...
    if (entry.constant || entry.will || entry.rest || entry.daily || entry.weekly) {
      ...
      if (entry.constant && !hidden.has('constant'))
        tempList.items.push({ type: 'itemSpell', name: `Constant:`, entry: entry.constant.join(', ') })
      ...
      if (entry.rest && !hidden.has('rest')) {
        for (let lvl = 9; lvl > 0; lvl--) {
          ...
          const lvlEach = `${lvl}e`
          if (rest[lvlEach])
            tempList.items.push({ type: 'itemSpell', name: `${lvl}/rest each:`, entry: rest[lvlEach].join(', ') })
        }
      }
      if (entry.daily && !hidden.has('daily')) {
        for (let lvl = 9; lvl > 0; lvl--) {
          ...
          const lvlEach = `${lvl}e`
          if (daily[lvlEach])
            tempList.items.push({ type: 'itemSpell', name: `${lvl}/day each:`, entry: daily[lvlEach].join(', ') })
        }
      }
      if (entry.weekly && !hidden.has('weekly')) {
        for (let lvl = 9; lvl > 0; lvl--) {
          ...
          const lvlEach = `${lvl}e`
          if (weekly[lvlEach])
            tempList.items.push({ type: 'itemSpell', name: `${lvl}/week each:`, entry: weekly[lvlEach].join(', ') })
        }
      }
      ...
      for (let lvl = 0; lvl < 10; ++lvl) {
        ...
        if (spells) {
          ...
          if (spells.lower && spells.lower !== lvl) {
            ...
            if (slots >= 0)
              slotsAtWill = slots > 0 ? ` (${slots} ${LEVEL.spLevelToFull(lvl)}-level slot${slots > 1 ? 's' : ''})` : ``
          }
  ...
  this._renderAbilityDc = function(entry, textStack, meta, options) {
    ...
    textStack[0] += ` save DC</b> = 8 + your proficiency bonus + your ${ABILITIES.attrChooseToFull(entry.attributes)}</div>`
    this._renderSuffix(entry, textStack, meta, options)
  }
  ...
  this._renderAbilityAttackMod = function(entry, textStack, meta, options) {
    ...
    textStack[0] += `<div class="text-center"><b>`
    this._recursiveRender(entry.name, textStack, meta)
    textStack[0] += ` attack modifier</b> = your proficiency bonus + your ${ABILITIES.attrChooseToFull(entry.attributes)}</div>`
    this._renderSuffix(entry, textStack, meta, options)
  }
  ...
  this._renderAbilityGeneric = function(entry, textStack, meta, options) {
    ...
    textStack[0] += `${entry.text}${entry.attributes ? ` ${ABILITIES.attrChooseToFull(entry.attributes)}` : ''}</div>`
    this._renderSuffix(entry, textStack, meta, options)
  }
  ...
  this._renderActions = function(entry, textStack, meta, options) {
    this._handleTrackTitles(entry.name)
    textStack[0] += `<${this.wrapperTag} class="${Renderer.HEAD_2}"><span class="rd__h rd__h--3" data-title-index="${this
      ._headerIndex++}" ${this._getEnumeratedTitleRel(entry.name)}><span class="entry-title-inner">${entry.name}.</span></span> `
    const len = entry.entries.length
  ...
  this._renderAttack = function(entry, textStack, meta, options) {
    this._renderPrefix(entry, textStack, meta, options)
    textStack[0] += `<i>${ATTACK.TYPE_TO_FULL.A(entry.attackType)}:</i> `
  ...
  this._renderDataHeader = function(textStack, name) {
    ...
    textStack[0] += `<thead><tr><th class="rd__data-embed-header" colspan="6" onclick="((ele) => {
          ...
          })(this)"><span style="display: none;" class="rd__data-embed-name">${name}</span>
          <span class="rd__data-embed-toggle">[\u2013]</span></th></tr></thead><tbody>`
  }
  ...
  this._renderGallery = function(entry, textStack, meta, options) {
    ...
    for (let i = 0; i < len; ++i) {
      const img = _.cloneDeep(entry.images[i])
  ...
  this._renderCode = function(entry, textStack, meta, options) {
    textStack[0] += `<button class="btn btn-default btn-xs mb-1" onclick="{
        const $e = $(this).next('pre'); 
        MiscUtil.pCopyTextToClipboard($e.text());JqueryUtil.showCopiedEffect($e)
      }">Copy Code</button>
      ...
      if (s[0] === '@') {
        ...
        switch (tag) {
          ...
          case '@scaledice': {
            ...
            const progressionParse = parseNumberRange(progression, 1, 9)
          ...
          case '@loader': {
            ...
            textStack[0] += `<span 
              ...
              class="rd__wrp-loadbrew--ready" title="Click to install homebrew">
                ${name}
  ...
  this._renderLink_getHoverString = function(entry) {
    ...
    if (this._isAddHandlers)
      return `onmouseover="Renderer.hover.pHandleLinkMouseOver(event, this, '${entry.href.hover.page}', '${
        ...
        onmousemove="Renderer.hover.handleLinkMouseMove(event, this)"  ${Renderer.hover.getPreventTouchString()}`
    else return ''
  }
...
Renderer.HOVER_TAG_TO_PAGE = {
  spell: PAGES.SPELLS,
  item: PAGES.ITEMS,
  creature: PAGES.BESTIARY,
  condition: PAGES.CONDITIONS_DISEASES,
  disease: PAGES.CONDITIONS_DISEASES,
  background: PAGES.BACKGROUNDS,
  race: PAGES.RACES,
  optfeature: PAGES.OPT_FEATURES,
  reward: PAGES.REWARDS,
  feat: PAGES.FEATS,
  psionic: PAGES.PSIONICS,
  object: PAGES.OBJECTS,
  cult: PAGES.CULTS_BOONS,
  boon: PAGES.CULTS_BOONS,
  trap: PAGES.TRAPS_HAZARDS,
  hazard: PAGES.TRAPS_HAZARDS,
  deity: PAGES.DEITIES,
  variantrule: PAGES.VARIATNRULES
}
...
Renderer.getEntryDice = function(entry, name, isAddHandlers = true) {
  ...
  if (entry.rollable === true) {
    const toPack = _.cloneDeep(entry)
    if (typeof toPack.toRoll !== 'string') {
      // handle legacy format
      toPack.toRoll = Renderer.legacyDiceToString(toPack.toRoll)
    }

    const handlerPart = isAddHandlers
      ? `onmousedown="event.preventDefault()" onclick="Renderer.dice.pRollerClickUseData(event, this)" data-packed-dice=${pack(
          toPack
        )}`
      : ''
    return `<span class="roller render-roller" ${
      name ? `title="${name ? `${name.escapeQuotes()}` : ''}"` : ''
    } ${handlerPart}>${toDisplay}</span>`
...
Renderer.legacyDiceToString = function(array) {
  let stack = ''
  array.forEach((r) => {
    stack += `${r.neg ? '-' : stack === '' ? '' : '+'}${r.number || 1}d${r.faces}${
...
Renderer.getAbilityData = function(abArr) {
  function doRenderOuter(abObj) {
    ...
    function handleAllAbilities(abObj, targetList) {
      _.cloneDeep(ABILITIES._.LIST_ABBREVIATIONS)
        .sort((a, b) => sort(abObj[b] || 0, abObj[a] || 0))
    ...
    function handleAbilitiesChoose() {
      if (abObj.choose != null) {
        ...
        if (ch.weighted) {
          ...
          const startText = froms.length === 6 ? `Choose ` : `From ${froms.joinConjunct(', ', ' and ')} choose `
          toConvertToText.push(`${startText}${areIncrease.concat(areReduce).joinConjunct(', ', ' and ')}`)
          toConvertToShortText.push(
            `${froms.length === 6 ? 'Any combination ' : ''}${areIncreaseShort.concat(areReduceShort).join('/')}${
...
Renderer.utils = {
  ...
  getNameTr: (it, opts) => {
    ...
    let dataPart = ''
    if (opts.page) {
      const hash = HASH_BUILDER[opts.page](it)
    ...
    return `<tr>
            ...
            <span 
              class="stats-name copyable" 
              onmousedown="event.preventDefault()" 
              onclick="Renderer.utils._pHandleNameClick(this)">${opts.prefix || ''}${it._displayName || it.name}${opts.suffix ||
      ''}</span>
						${opts.pronouncePart || ''}
						${
              ExtensionUtil.ACTIVE
                ? `<button 
                  title="Send to Foundry (SHIFT for Temporary Import)" 
                  class="btn btn-xs btn-default btn-stats-name" 
                  onclick="ExtensionUtil.pDoSendStats(event, this)"><span class="glyphicon glyphicon-send"/></button>`
                : ''
            }
					</div>
					<span class="stats-source ${
            it.source
              ? `${sourceJSONToColor(it.source)}" title="${sourceJSONToFull(it.source)}${Renderer.utils.getSourceSubText(it)}`
              : ''
          }" style="${it.source ? `color: ${BrewUtil.sourceJsonToColor(it.source)};` : ''}">
						${it.source ? sourceJSONToAbv(it.source) : ''}${opts.isAddPageNum && it.page > 0 ? ` p${it.page}` : ''}
					</span>
				</div>
			</th>
		</tr>`
  ...
  getExcludedTr(it, dataProp) {
    ...
    // return isExcluded
    //   ? `<tr><td colspan="6" class="pt-3 text-center text-danger">
    //       <b><i>Warning: This content has been blacklisted.</i></b>
    //     </td></tr>`
    //   : ''
  },
  ...
  async _pHandleNameClick(ele) {
    ...
    console.warn('Name click copy effect not implemented')
  ...
  _getPageTrText: (it) => {
      ...
      return `${introText} ${it[prop]
        .map((as) => {
          if (as.entry) return Renderer.get().render(as.entry)
            ...
            return `<i title="${sourceJSONToFull(as.source)}">${sourceJSONToAbv(as.source)}</i>${
    ...
    const baseText =
      it.page > 0
        ? `<b>Source:</b> <i title="${sourceJSONToFull(it.source)}${sourceSub}">${sourceJSONToAbv(
            it.source
    ...
    return `${[baseText, addSourceText, otherSourceText, externalSourceText].filter((it) => it).join('. ')}${
      baseText && (addSourceText || otherSourceText || externalSourceText) ? '.' : ''
    }`
  },
  ...
  bindTabButtons: (...tabButtons) => {
    ...
    const toAdd = tabButtons.map((tb, i) => {
      const toSel =
        (!Renderer.utils._prevTab && i === 0) || (Renderer.utils._prevTab && Renderer.utils._prevTab.label === tb.label)
      ...
      .map((pr) => {
        return Object.entries(pr)
          ...
          .map(([k, v]) => {
            ...
            switch (k) {
              case 'level':
                return listMode ? `Lvl ${v.level}` : `${Parser.getOrdinalForm(v.level)} level`
              case 'pact':
                return Parser.prereqPactToFull(v)
              case 'patron':
                return listMode ? `${Parser.prereqPatronToShort(v)} patron` : `${v} patron`
              case 'spell':
                return listMode
                  ? v
                      .map((x) =>
              ...
              case 'other':
                return listMode ? 'Special' : Renderer.get().render(v)
              case 'race': {
                return v
                  .map((it, i) => {
                    if (listMode) {
              ...
              case 'ability': {
                ...
                const abilityOptions = v.map((abMeta) => {
                  ...
                  return listMode
                    ...
                    : abList.map((ab) => Parser.attAbvToFull(ab)).joinConjunct(', ', ' and ')
                })
                return listMode
                  ? `${abilityOptions.join('/')} 13+`
                  : `${abilityOptions.joinConjunct(hadMultipleInner ? '; ' : ', ', ' or ')} 13 or higher`
              }
              case 'proficiency': {
              ...
              case 'spellcasting':
                return listMode ? 'Spellcasting' : 'The ability to cast at least one spell'
              default:
...
Renderer.feat = {
  ...
  getCompactRenderedString: (feat) => {
    ...
    renderStack.push(`
			...
			${Renderer.utils.getNameTr(feat, { isAddPageNum: true, page: PAGES.FEATS })}
...
Renderer.spell = {
  getCompactRenderedString: (spell) => {
    ...
    renderStack.push(`
			...
			${Renderer.utils.getNameTr(spell, { isAddPageNum: true, page: PAGES.SPELLS })}
			<tr><td colspan="6">
				<table class="summary striped-even">
					...
					<tr>
						<td colspan="1">${LEVEL.spLevelToFull(spell.level)}${Parser.spMetaToFull(spell.meta)}</td>
  ...
  initClasses(spell, brewSpellClasses) {
    ...
    if (
      ...
      spell.classes.fromClassList.filter((c) => c.name === Renderer.spell.STR_WIZARD && c.source === SOURCES.CODE.PHB).length
    ) {
      if (!spell.classes.fromSubclass) spell.classes.fromSubclass = []
      spell.classes.fromSubclass.push({
        class: { name: Renderer.spell.STR_FIGHTER, source: SOURCES.CODE.PHB },
        subclass: { name: Renderer.spell.STR_ELD_KNIGHT, source: SOURCES.CODE.PHB }
      })
      spell.classes.fromSubclass.push({
        class: { name: Renderer.spell.STR_ROGUE, source: SOURCES.CODE.PHB },
        subclass: { name: Renderer.spell.STR_ARC_TCKER, source: SOURCES.CODE.PHB }
      })
      if (spell.level > 4) {
    ...
    if (
      spell.classes &&
      spell.classes.fromClassList &&
      spell.classes.fromClassList.filter((c) => c.name === Renderer.spell.STR_CLERIC && c.source === SOURCES.CODE.PHB).length
    ) {
      if (!spell.classes.fromSubclass) {
        spell.classes.fromSubclass = []
        spell.classes.fromSubclass.push({
          class: { name: Renderer.spell.STR_SORCERER, source: SOURCES.CODE.PHB },
          subclass: { name: Renderer.spell.STR_DIV_SOUL, source: SOURCES.CODE.XGE }
        })
      } else if (
        !spell.classes.fromSubclass.find(
          (it) =>
            it.class.name === Renderer.spell.STR_SORCERER &&
            it.class.source === SOURCES.CODE.PHB &&
            it.subclass.name === Renderer.spell.STR_DIV_SOUL &&
            it.subclass.source === SOURCES.CODE.XGE
        )
      ) {
        spell.classes.fromSubclass.push({
          class: { name: Renderer.spell.STR_SORCERER, source: SOURCES.CODE.PHB },
      ...
      spell.classes.fromSubclass.push({
        class: { name: Renderer.spell.STR_SORCERER, source: SOURCES.CODE.PHB },
        subclass: { name: Renderer.spell.STR_FAV_SOUL_V2, source: SOURCES.CODE.UAS }
      })
      spell.classes.fromSubclass.push({
        class: { name: Renderer.spell.STR_SORCERER, source: SOURCES.CODE.PHB },
        subclass: { name: Renderer.spell.STR_FAV_SOUL_V3, source: SOURCES.CODE.UARSC }
      })
    }
    ...
    if (spell.classes && spell.classes.fromClassList && spell.classes.fromClassList.find((it) => it.name === 'Wizard')) {
      if (spell.level === 0) {
        ...
        ;(spell.races || (spell.races = [])).push({
          name: 'Elf (High)',
          source: SOURCES.CODE.PHB,
          baseName: 'Elf',
          baseSource: SOURCES.CODE.PHB
        })
        // add arcana cleric
        ;(spell.classes.fromSubclass = spell.classes.fromSubclass || []).push({
          class: { name: Renderer.spell.STR_CLERIC, source: SOURCES.CODE.PHB },
          subclass: { name: 'Arcana', source: SOURCES.CODE.SCAG }
        })
      }
    ...
    if (spell.classes && spell.classes.fromClassList && spell.classes.fromClassList.find((it) => it.name === 'Druid')) {
      if (spell.level === 0) {
        ...
        ;(spell.classes.fromSubclass = spell.classes.fromSubclass || []).push({
          class: { name: Renderer.spell.STR_CLERIC, source: SOURCES.CODE.PHB },
          subclass: { name: 'Nature', source: SOURCES.CODE.PHB }
        })
      }
    }
...
Renderer.condition = {
  getCompactRenderedString: (cond) => {
    ...
    renderStack.push(`
			...
			${Renderer.utils.getNameTr(cond, { isAddPageNum: true, page: PAGES.CONDITIONS_DISEASES })}
...
Renderer.background = {
  getCompactRenderedString(bg) {
    return `
		...
		${Renderer.utils.getNameTr(bg, { isAddPageNum: true, page: PAGES.BACKGROUNDS })}
		<tr class="text"><td colspan="6">
        ...
        const toJoin = Object.keys(profGroup)
          ...
          .map((k, i) => {
            if (k === 'choose') {
              ...
              return `${short ? `${i === 0 ? 'C' : 'c'}hoose ` : ''}${choose.count || 1} ${
                short ? `of` : `from`
...
Renderer.optionalfeature = {
  ...
  getPreviouslyPrintedText(it) {
    return it.data && it.data.previousVersion
      ? `<tr><td colspan="6"><p>${Renderer.get().render(
          `{@i An earlier version of this ${Parser.optFeatureTypeToFull(it.featureType)} is available in }${sourceJSONToFull(
            it.data.previousVersion.source
          )} {@i as {@optfeature ${it.data.previousVersion.name}|${it.data.previousVersion.source}}.}`
        )}</p></td></tr>`
      : ''
  },
  ...
  getCompactRenderedString: (it) => {
    ...
    renderStack.push(`
			...
			${Renderer.utils.getNameTr(it, { isAddPageNum: true, page: PAGES.OPT_FEATURES })}
			<tr class="text"><td colspan="6">
...
Renderer.reward = {
  ...
  getCompactRenderedString: (reward) => {
    return `
			...
			${Renderer.utils.getNameTr(reward, { isAddPageNum: true, page: PAGES.REWARDS })}
...
Renderer.race = {
  getCompactRenderedString: (race) => {
    ...
    renderStack.push(`
			...
			${Renderer.utils.getNameTr(race, { isAddPageNum: true, page: PAGES.RACES })}
			<tr><td colspan="6">
				<table class="summary striped-even">
					<tr>
...
Renderer.deity = {
  ...
  getOrderedParts(deity, prefix, suffix) {
    ...
    const allKeys = Object.keys(parts).sort(sortLower)
...
Renderer.object = {
  getCompactRenderedString: (obj) => {
    ...
    return `
			...
			${Renderer.utils.getNameTr(obj, { isAddPageNum: true, page: PAGES.OBJECTS })}
			<tr><td colspan="6">
				<table class="summary striped-even">
					<tr>
					...
					${
            ABILITIES._.LIST_ABBREVIATIONS.some((ab) => obj[ab] != null)
              ? `
					<tr>${ABILITIES._.LIST_ABBREVIATIONS.map((it) => `<td colspan="2" class="text-center">${it.toUpperCase()}</td>`).join('')}</tr>
					<tr>${ABILITIES._.LIST_ABBREVIATIONS.map(
            (it) => `<td colspan="2" class="text-center">${Renderer.utils.getAbilityRoller(obj, it)}</td>`
          ).join('')}</tr>
					`
              : ''
          }
...
Renderer.traphazard = {
      ...
      return renderer.render(
        {
          entries: [
            ...
            },
            {
              ...
              name: 'Initiative',
              entries: [
                `The trap acts on ${Parser.trapInitToFull(it.initiative)}${it.initiativeNote ? ` (${it.initiativeNote})` : ''}.`
              ]
            },
            it.eActive
              ? {
  ...
  getCompactRenderedString: (it) => {
    ...
    return `
			...
			${Renderer.utils.getNameTr(it, { isAddPageNum: true, page: PAGES.TRAPS_HAZARDS })}
			${
        subtitle
  ...
  getCompactRenderedString: (it) => {
    ...
    if (it._type === 'c') {
      ...
      return `
			...
			${Renderer.utils.getNameTr(it, { isAddPageNum: true, page: PAGES.CULTS_BOONS })}
...
Renderer.monster = {
  getLegendaryActionIntro: (mon, renderer = Renderer.get()) => {
    function getCleanName() {
      ...
      const cleanDragons = base.replace(/(?:adult|ancient|young) \w+ (dragon|dracolich)/gi, '$1')
    ...
    if (mon.legendaryHeader) {
      ...
      return `${mon.isNamedCreature ? '' : 'The '}${legendaryName} can take ${legendaryActions} legendary action${
  ...
  getDragonCasterVariant(renderer, dragon) {
    ...
    const maxSpellLevel = Math.floor(CR.crToNumber(dragon.cr) / 3)
    const exampleSpells = getExampleSpells(maxSpellLevel, dragon.dragonCastingColor)
    const levelString =
      maxSpellLevel === 0
        ? `${chaMod === 1 ? 'This' : 'These'} spells are Cantrips.`
        : `${chaMod === 1 ? 'The' : 'Each'} spell's level can be no higher than ${LEVEL.spLevelToFull(maxSpellLevel)}.`
    const v = {
      type: 'variant',
      ...
      entries: [
        ...
        `{@i This dragon can innately cast ${Parser.numberToText(chaMod)} spell${chaMod === 1 ? '' : 's'}, once per day${
          chaMod === 1 ? '' : ' each'
        }, requiring no material components. ${levelString} The dragon's spell save DC is ${pb +
          chaMod +
          8}, and it has {@hit ${pb + chaMod}} to hit with spell attacks. See the {@filter spell page|spells|level=${[
  ...
  getCrScaleTarget($btnScaleCr, initialCr, cbRender, isCompact) {
    ...
    $sld
      ...
      .slider('pips', subOpts)
      .slider('float', subOpts)

    $sld.slider().on('slidechange', () => {
      const ix = $sld.slider('value')
      cbRender(CR.crToNumber(Parser.CRS[ix]))
      $body.off(evtName)
      cleanSliders()
    })
  ...
  getTypeAlignmentPart(mon) {
    return `${mon.level ? `${Parser.getOrdinalForm(mon.level)}-level ` : ''}${Parser.sizeAbvToFull(mon.size)} ${
      Parser.monTypeToFullObj(mon.type).asText
    }${mon.alignment ? `, ${Parser.alignmentListToFull(mon.alignment).toLowerCase()}` : ''}`
  },
  getSavesPart(mon) {
  ...
  getCompactRenderedString: (mon, renderer, options = {}) => {
    ...
    const renderStack = []
    const isCrHidden = CR.crToNumber(mon.cr) === 100

    renderStack.push(`
			${Renderer.utils.getExcludedTr(mon, 'monster')}
			${Renderer.utils.getNameTr(mon, { isAddPageNum: true, page: PAGES.BESTIARY })}
  ...
  getRenderedHp: (hp, isPlainText) => {
    ...
    if (/^\d+d1$/.exec(hp.formula)) {
      ...
      return `${maxStr ? `<span title="${maxStr}" class="help--subtle">` : ''}${hp.average}${
        maxStr ? '</span>' : ''
      } ${Renderer.get().render(`({@dice ${hp.formula}|${hp.formula}|Hit Points})`)}`
    }
  },
  ...
  getOrderedTraits: (mon, renderer) => {
    let trait = mon.trait ? _.cloneDeep(mon.trait) : null
    if (mon.spellcasting) {
  ...
  getSkillsString(renderer, mon) {
    ...
    function doSortMapJoinSkillKeys(obj, keys, joinWithOr) {
      const toJoin = keys
        ...
        .map(
          (s) =>
            `<span data-mon-skill="${s.toTitleCase()}|${obj[s]}">${renderer.render(
              `{@skill ${s.toTitleCase()}}`
  ...
  getTokenUrl(mon) {
    return (
      mon.tokenUrl ||
      UrlUtil.link(`${Renderer.get().baseUrl}img/${sourceJSONToAbv(mon.source)}/${Parser.nameToTokenName(mon.name)}.png`)
    )
  },
  ...
  getFluff(mon, legendaryMeta, fluffJson) {
    ...
    const rawFluff =
      predefined || (fluffJson || { monster: [] }).monster.find((it) => it.name === mon.name && it.source === mon.source)
...
Renderer.item = {
  ...
  _sortProperties(a, b) {
    return sort(Renderer.item.propertyMap[a].name, Renderer.item.propertyMap[b].name)
  },

  _getPropertiesText(item) {
    if (item.property) {
      const renderedProperties = item.property.sort(Renderer.item._sortProperties).map((prop) => {
  ...
  getDamageAndPropertiesText(item) {
    const damageParts = []
    ...
    if (item.vehSpeed || item.capCargo || item.capPassenger || item.crew || item.vehAc || item.vehHp || item.vehDmgThresh) {
      ...
      const vehPartMiddle =
        item.capCargo || item.capPassenger
          ? `Carrying Capacity: ${[
              item.capCargo
                ? `${Parser.numberToFractional(item.capCargo)} ton${item.capCargo === 0 || item.capCargo > 1 ? 's' : ''} cargo`
                : null,
  ...
  getTypeRarityAndAttunementText(item) {
    const typeRarity = [
      ...
      [item.tier, item.rarity && Renderer.item.doRenderRarity(item.rarity) ? item.rarity : '']
        .map((it) => (it || '').trim())
        .filter((it) => it)
        .join(', ')
    ]
  ...
  getHtmlAndTextTypes(item) {
    ...
    if (item.weaponCategory) {
      typeListHtml.push(
        `${item.weaponCategory} Weapon${item.baseItem ? ` (${Renderer.get().render(`{@item ${item.baseItem}`)})` : ''}`
      )
  ...
  getRenderedEntries(item, isCompact) {
    ...
    if (item._fullEntries || (item.entries && item.entries.length)) {
      const entryList = _.cloneDeep({ type: 'entries', entries: item._fullEntries || item.entries })
    ...
    if (item._fullAdditionalEntries || item.additionalEntries) {
      const additionEntriesList = _.cloneDeep({
        type: 'entries',
    ...
    if (!isCompact && item.lootTables) {
      renderStack.push(
        `<div><span class="bold">Found On: </span>${item.lootTables
          .sort(sortLower)
  ...
  getCompactRenderedString(item) {
    ...
    return `
		...
		${Renderer.utils.getNameTr(item, { isAddPageNum: true, page: PAGES.ITEMS })}
  ...
  _addAdditionalEntries(e) {
    ...
    Renderer.item._additionalEntriesMap[e.appliesTo] = _.cloneDeep(e.entries)
  },
  ...
  async pBuildList(opts) {
    ...
    if (Renderer.item._builtLists[kBlacklist]) {
      const cached = opts.isAddGroups
        ? Renderer.item._builtLists[kBlacklist]
        : Renderer.item._builtLists[kBlacklist].filter((it) => !it._isItemGroup)
    ...
    const [genericVariants, linkedLootTables] = await Renderer.item._pGetAndProcGenericVariants(
      await DataUtil.loadJSON(magicVariantUrl),
      true
    )
  ...
  _initFullEntries(item) {
    item._fullEntries = item._fullEntries || (item.entries ? _.cloneDeep(item.entries) : [])
  },

  _initFullAdditionalEntries(item) {
    item._fullAdditionalEntries =
      item._fullAdditionalEntries || (item.additionalEntries ? _.cloneDeep(item.additionalEntries) : [])
  },
  ...
  _createSpecificVariants(baseItems, genericVariants, linkedLootTables) {
    ...
    function hasExcludedProperty(baseItem, genericVariant) {
      ...
      return !!Object.keys(curExcludes).find((key) => {
        if (Array.isArray(curExcludes[key])) {
          return Array.isArray(baseItem[key])
            ? baseItem[key].find((it) => curExcludes[key].includes(it))
            : curExcludes[key].includes(baseItem[key])
        }
        return Array.isArray(baseItem[key])
          ? baseItem[key].find((it) => curExcludes[key] === it)
          : curExcludes[key] === baseItem[key]
      })
    }
    ...
    function createSpecificVariant(baseItem, genericVariant) {
      ...
      if (baseItem.source !== SOURCES.CODE.PHB && baseItem.source !== SOURCES.CODE.DMG) {
        ...
        specificVariant._fullEntries.unshift(`{@note The base item can be found in ${sourceJSONToFull(baseItem.source)}.}`)
      }
      ...
      if (
        linkedLootTables &&
        linkedLootTables[specificVariant.source] &&
        linkedLootTables[specificVariant.source][specificVariant.name]
      ) {
        ;(specificVariant.lootTables = specificVariant.lootTables || []).push(
          ...linkedLootTables[specificVariant.source][specificVariant.name]
        )
      }
  ...
  _genericVariants_addInheritedPropertiesToSelf(genericVariant) {
    for (const prop in genericVariant.inherits) {
      ...
      if (val == null) delete genericVariant[prop]
        ...
        if (Array.isArray(genericVariant[prop]) && Array.isArray(val))
          genericVariant[prop] = _.cloneDeep(genericVariant[prop]).concat(val)
    ...
    if (!genericVariant.entries && genericVariant.inherits.entries) {
      genericVariant.entries = _.cloneDeep(
        genericVariant.inherits.entries.map((ent) =>
          typeof ent === 'string' ? Renderer.applyProperties(ent, genericVariant.inherits) : ent
        )
      )
    }
  ...
  enhanceItem(item) {
    ...
    if (item._isItemGroup) {
      Renderer.item._initFullEntries(item)
      item._fullEntries.push('Multiple variants of this item exist, as listed below:', {
        type: 'list',
  ...
  modifierPostToPre(item) {
    ...
    if (m) return Object.assign(_.cloneDeep(item), { name: `${m[2]} ${m[1]}` })
    else return null
  },
  ...
  populatePropertyAndTypeReference: () => {
    ...
    return new Promise((resolve, reject) => {
      DataUtil.loadJSON(`${Renderer.get().baseUrl}data/items-base.json`).then((data) => {
  ...
  async getAllIndexableItems(rawVariants, rawBaseItems) {
    ...
    genericAndSpecificVariants.forEach((item) => {
      ...
      const revName = Renderer.item.modifierPostToPre(_.cloneDeep(item))
...
Renderer.psionic = {
  ...
  getDisciplineText: (psionic, renderer) => {
    ...
    return `${Renderer.psionic.getDescriptionString(psionic, renderer)}${Renderer.psionic.getFocusString(
      psionic,
      renderer
    )}${modeStringArray.join('')}`
  },
  ...
  getCompactRenderedString: (psionic) => {
    ...
    const typeOrderStr =
      psionic.type === 'T' ? Parser.psiTypeToFull(psionic.type) : `${psionic.order} ${Parser.psiTypeToFull(psionic.type)}`
    const bodyStr =
      psionic.type === 'T'
        ? Renderer.psionic.getTalentText(psionic, renderer)
        : Renderer.psionic.getDisciplineText(psionic, renderer)

    return `
			${Renderer.utils.getExcludedTr(psionic, 'psionic')}
			${Renderer.utils.getNameTr(psionic, { isAddPageNum: true, page: PAGES.PSIONICS })}
			<tr class="text"><td colspan="6">
...
Renderer.variantrule = {
  getCompactRenderedString(rule) {
    const cpy = _.cloneDeep(rule)
    delete cpy.name
    return `
			${Renderer.utils.getExcludedTr(rule, 'variantrule')}
			${Renderer.utils.getNameTr(rule, { isAddPageNum: true, page: PAGES.VARIATNRULES })}
			<tr><td colspan="6">
			${Renderer.get()
        .setFirstSection(true)
        .render(cpy)}
...
Renderer.table = {
  getCompactRenderedString(it) {
    ...
    const cpy = _.cloneDeep(it)
    delete cpy.name
    return `
			${Renderer.utils.getExcludedTr(it, 'table')}
			${Renderer.utils.getNameTr(it, { isAddPageNum: true, page: PAGES.TABLES })}
			<tr><td colspan="6">
			${Renderer.get()
        .setFirstSection(true)
        .render(it)}
  ...
  _getRenderedString_ship(veh, opts) {
    ...
    function getSectionHpPart(sect, each) {
      ...
      return `
				...
				<div><b>Hit Points</b> ${sect.hp}${each ? ` each` : ''}${sect.dt ? ` (damage threshold ${sect.dt})` : ''}${
        sect.hpNote ? `; ${sect.hpNote}` : ''
      }</div>
			`
    }
    ...
    function getMovementSection(move) {
      ...
      return `
				<tr class="mon__stat-header-underline"><td colspan="6"><span>${move.isControl ? `Control and ` : ''}Movement: ${
        move.name
      }</span></td></tr>
				<tr><td colspan="6">
    ...
    function getWeaponSection(weap) {
      return `
				<tr class="mon__stat-header-underline"><td colspan="6"><span>Weapons: ${weap.name}${
        weap.count ? ` (${weap.count})` : ''
      }</span></td></tr>
				<tr><td colspan="6">
      ...
      veh.dimensions ? ` (${veh.dimensions.join(' by ')})` : ''
          ...
          veh.capCargo
            ? `<div><b>Cargo Capacity</b> ${
                typeof veh.capCargo === 'string' ? veh.capCargo : `${veh.capCargo} ton${veh.capCargo === 1 ? '' : 's'}`
              }</div>`
            : ''
        }
...
Renderer.action = {
  getCompactRenderedString(it) {
    const cpy = _.cloneDeep(it)
    delete cpy.name
    return `${Renderer.utils.getExcludedTr(it, 'action')}${Renderer.utils.getNameTr(it, {
      isAddPageNum: true,
      page: PAGES.ACTIONS
    })}
...
Renderer.hover = {
  ...
  _doInit() {
    if (!Renderer.hover._isInit) {
      ...
      ContextUtil.doInitContextMenu(
        'hoverBorder',
        (evt, ele, $invokedOn, $selectedMenu, _, windowMeta) => {
          // windowMeta for future use for more options
  ...
  async pHandleLinkMouseOver(evt, ele, page, source, hash, preloadId) {
    ...
    if (page === PAGES.BESTIARY) {
      const renderFn = Renderer.hover._pageToRenderFn(page)
      $content.on('click', '.mon__btn-scale-cr', (evt) => {
        evt.stopPropagation()
        const $this = $(this)
      ...
      $content.on('click', '.mon__btn-reset-cr', async () => {
        toRender = await Renderer.hover.pCacheAndGet(page, source, hash)
        $content.empty().append(renderFn(toRender))
        meta.windowMeta.$windowTitle.text(toRender._displayName || toRender.name)
      })
  ...
  getShowWindow($content, position, opts) {
    ...
    const $brdrBtm = $(
      `<div class="hoverborder hoverborder--btm ${
        opts.isBookContent ? 'hoverborder-book' : ''
      }"><div class="hoverborder__resize-s"/></div>`
    )
    ...
    const $brdrTop = $(
      `<div class="hoverborder hoverborder--top ${opts.isBookContent ? 'hoverborder-book' : ''}" ${
        opts.isPermanent ? `data-perm="true"` : ''
      }/>`
    )
    ...
    function isOverHoverTarget(evt, target) {
      return (
        EventUtil.getClientX(evt) >= target.left &&
          ...
          if (drag.type === 9) {
            ...
            if (this._dmScreen) {
              ...
              if (isOverHoverTarget(evt, target)) {
                if (preLoaded && preLoaded._isScaledCr != null)
                  panel.doPopulate_StatsScaledCr(page, source, hash, preLoaded.cr.cr || preLoaded.cr)
    ...
    if (opts.pageUrl) {
      const $btnGotoPage = $(
        `<a class="top-border-icon glyphicon glyphicon-modal-window" style="margin-right: 2px;" title="Go to Page" href="${opts.pageUrl}"></a>`
      ).appendTo($brdTopRhs)
    }
    ...
    const $btnPopout = $(
      `<span class="top-border-icon glyphicon glyphicon-new-window hvr__popout" style="margin-right: 2px;" title="Open as Popup Window"></span>`
    )
      .on('click', (evt) => {
        evt.stopPropagation()
        const h = $content.height()
        const win = open('', opts.title || '', `width=600,height=${h}location=0,menubar=0,status=0,titlebar=0,toolbar=0`)
        win.document.write(`
					<!DOCTYPE html>
					<html lang="en" class="${
            styleSwitcher && styleSwitcher.getActiveStyleSheet() === StyleSwitcher.STYLE_NIGHT ? StyleSwitcher.NIGHT_CLASS : ''
          }"><head>
    ...
    $hov
      .append($brdrTopResize)
      .append($brdrTopRightResize)
      .append($brdrRightResize)
      .append($brdrBottomRightResize)
      .append($brdrBtmLeftResize)
      .append($brdrLeftResize)
      .append($brdrTopLeftResize)
  ...
  _addToCache: (page, source, hash, item) => {
    ...
    ;((Renderer.hover._linkCache[page] = Renderer.hover._linkCache[page] || [])[source] =
  ...
  _isCached: (page, source, hash) => {
    return (
      Renderer.hover._linkCache[page] && Renderer.hover._linkCache[page][source] && Renderer.hover._linkCache[page][source][hash]
    )
  },
  ...
  async pCacheAndGet(page, source, hash) {
    ...
    function populate(data, listProp, itemModifier) {
      data[listProp].forEach((it) => {
        const itHash = HASH_BUILDER[page](it)
    ...
    async function _pLoadSingleBrew(listProps, itemModifier) {
      const brewData = await BrewUtil.pAddBrewData()
      listProps = Array.isArray(listProps) ? listProps : [listProps]
    ...
    function _handleSingleData(data, listProps, itemModifier) {
      if (Array.isArray(listProps)) listProps.forEach((p) => populate(data, p, itemModifier))
    ...
    function _classes_indexFeatures(cls) {
      ...
      UrlUtil.class
        ...
        .forEach((it) => Renderer.hover._addToCache(PAGES.CLASSES, it.source.source || it.source, it.hash, it.entry))
    }
    ...
    switch (page) {
      ...
      case PAGES.BESTIARY:
        return pLoadMultiSource(page, `data/bestiary/`, 'monster')
      case PAGES.ITEMS: {
        const loadKey = PAGES.ITEMS
        ...
        if (!Renderer.hover._flagsCacheLoaded[loadKey] || !Renderer.hover._isCached(page, source, hash)) {
          Renderer.hover._psCacheLoading[loadKey] = (async () => {
            ...
            itemList.forEach((it) => {
              ...
              if (revName) Renderer.hover._addToCache(page, it.source, HASH_BUILDER[page](revName), it)
            })

            allItems.forEach((item) => {
              const itemHash = HASH_BUILDER[PAGES.ITEMS](item)
              Renderer.hover._addToCache(page, item.source, itemHash, item)
              const revName = Renderer.item.modifierPostToPre(item)
              if (revName) Renderer.hover._addToCache(page, item.source, HASH_BUILDER[page](revName), item)
            })
      ...
      case PAGES.RACES: {
        ...
        if (!Renderer.hover._flagsCacheLoaded[loadKey] || !Renderer.hover._isCached(page, source, hash)) {
          Renderer.hover._psCacheLoading[loadKey] = (async () => {
            ...
            merged.forEach((race) => {
              const raceHash = HASH_BUILDER[PAGES.RACES](race)
      ...
      case PAGES.TRAPS_HAZARDS:
        return pLoadSimple(page, 'trapshazards.json', ['trap', 'hazard'])
      case PAGES.VARIATNRULES:
        return pLoadSimple(page, 'variantrules.json', 'variantrule')
      case PAGES.CULTS_BOONS:
        return pLoadSimple(
          page,
          'cultsboons.json',
          ['cult', 'boon'],
          (listProp, item) => (item._type = listProp === 'cult' ? 'c' : 'b')
        )
      case PAGES.CONDITIONS_DISEASES:
        return pLoadSimple(
          page,
          'conditionsdiseases.json',
          ['condition', 'disease'],
          (listProp, item) => (item._type = listProp === 'condition' ? 'c' : 'd')
        )
      case PAGES.TABLES:
        return pLoadSimple(
          page,
          ...
          ['table', 'tableGroup'],
          (listProp, item) => (item._type = listProp === 'table' ? 't' : 'g')
        )
      case PAGES.VEHICLES:
  ...
  $getHoverContent_statsCode(toRender) {
    const cleanCopy = DataUtil.cleanJson(_.cloneDeep(toRender))
    const toRenderCode = {
      type: 'code',
  ...
  $getHoverContent_generic(toRender, isBookContent) {
    return $$`<table class="stats ${isBookContent ? 'stats--book' : ''}">${Renderer.hover.getGenericCompactRenderedString(
      toRender
    )}</table>`
  },
...
Renderer.dice = {
  ...
  bindDmScreenPanel(panel, title) {
    if (Renderer.dice._panel) {
      // there can only be one roller box
  ...
  async pRollerClickUseData(evt, ele) {
    ...
    if (options.length > 1) {
      chosenRollData = await new Promise((resolve) => {
        const cpy = _.cloneDeep(rollData)
    ...
    const rollDataCpy = _.cloneDeep(chosenRollData)
    ...
    if (rollData.prompt) {
      rollDataCpyToRoll = await new Promise((resolve) => {
        const sortedKeys = Object.keys(rollDataCpy.prompt.options).sort(sortLower)
        ...
        ContextUtil.doInitContextMenu(
          ...
          (mostRecentEvt, _1, _2, _3, invokedOnId) => {
            ...
            if (!fromScaling) {
              ...
              name = `${LEVEL.spLevelToFull(k)}-level cast`
              rollDataCpy.toRoll += `+${fromScaling}`
              resolve(rollDataCpy)
            }
          },
          [
            { text: rollDataCpy.prompt.entry, disabled: true },
            null,
            ...sortedKeys.map((it) => `${LEVEL.spLevelToFull(it)} level`)
          ]
        )
  ...
  rollerClick: (evtMock, ele, packed, name) => {
    ...
    function attemptToGetTitle() {
      ...
      titleMaybe = $(ele)
        ...
        .children(`tr`)
        .first()
        .find(`th.name .stats-name`)
        .text()
      if (titleMaybe) return titleMaybe.trim()
      if (UrlUtil.getCurrentPage() === PAGES.CHARACTERS) {
  ...
  _handleRoll2(tree, rolledBy, cbMessage) {
    ...
    if (tree) {
      ...
      const lbl =
        rolledBy.label && (!rolledBy.name || rolledBy.label.trim().toLowerCase() !== rolledBy.name.trim().toLowerCase())
          ? rolledBy.label
          : null
      ...
      $out.append(`
				<div class="out-roll-item" title="${title}">
					...
					<div class="out-roll-item-button-wrp">
						<button title="Copy to input" class="btn btn-default btn-xs btn-copy-roll" onclick="Renderer.dice._$iptRoll.val('${tree._asString.replace(
              /\s+/g,
              ''
...
Use <span class="out-roll-item-code">${PREF_MACRO} list</span> to list saved macros.<br>
          ...
          case 'list':
            if (checkLength(others, 0)) {
              Object.keys(Renderer.dice.storage).forEach((name) => {
                Renderer.dice._showMessage(
                  `<span class="out-roll-item-code">#${name}</span> \u2014 ${Renderer.dice.storage[name]}`,
                  Renderer.dice.SYSTEM_USER
                )
              })
            } else {
          ...
          case 'add': {
            if (checkLength(others, 2)) {
              ...
              if (name.includes(' ') || name.includes('#')) showInvalid()
                ...
                pSave().then(() =>
                  Renderer.dice._showMessage(
                    `Saved macro <span class="out-roll-item-code">#${name}</span>`,
                    Renderer.dice.SYSTEM_USER
                  )
                )
              }
          ...
          case 'remove':
            if (checkLength(others, 1)) {
              if (Renderer.dice.storage[others[0]]) {
                ...
                pSave().then(() =>
                  Renderer.dice._showMessage(
                    `Removed macro <span class="out-roll-item-code">#${others[0]}</span>`,
                    Renderer.dice.SYSTEM_USER
                  )
                )
              } else {
                Renderer.dice._showMessage(
                  `Macro <span class="out-roll-item-code">#${others[0]}</span> not found`,
                  Renderer.dice.SYSTEM_USER
                )
              }
            } else {
  ...
  _cleanOperators2(str) {
    // TODO doesn't handle unary minus
    function cleanExpressions(ipt) {
          ...
          switch (c) {
            ...
            case ')':
              if (curDepth === maxDepth) {
                const slice = [...ipt.slice(lastOpenIndex + 1, i)]
      ...
      function flatten(it) {
        if (it instanceof P) {
          ...
          throw new TypeError('Should never occur!')
        }
      }
      flatten(ipt)
    ...
    str = str
      ...
      .replace(/dl/g, 'l')
      .replace(/dh/g, 'h') // shorthand drop lowest/highest
      .replace(/\)\(/g, ')*(')
      .replace(/(\d)\(/g, '$1*(') // add multiplication signs
  ...
  _parse2(infix) {
    ...
    function infixToPostfix(infix) {
      ...
      for (let i = 0; i < infix.length; ++i) {
        ...
        if (tkn.isNumeric()) {
          ...
          while (
            OPS[o2] &&
            ((OPS[o1].assoc === 'L' && OPS[o1].precedence <= OPS[o2].precedence) ||
              (OPS[o1].assoc === 'R' && OPS[o1].precedence < OPS[o2].precedence))
          ) {
            handleOpPop()
            o2 = opStack.last()
          }
    ...
    function postfixToTree(postfix) {
      ...
      function Dice(num, faces, drop, dropType) {
        ...
        this._get = (meta, nextFn) => {
          ...
          if (drop != null) {
            ...
            handlePrO(meta, this)
            meta.text.push(
              `${prOpen}${inSlice.length ? `[${inSlice.join(']+[')}]` : ''}${
                outSlice.length ? `<span style="text-decoration: red line-through;">+[${outSlice.join(']+[')}]</span>` : ''
              }${prClose}`
            )
            meta.rawText.push(
              `${prOpen}${inSlice.length ? `[${inSlice.join(']+[')}]` : ''}${
                outSlice.length ? `+[${outSlice.join(']+[')}]` : ''
              }${prClose}`
            )
            handlePrC(meta, this)
      ...
      const ipt = postfix
        .replace(/[()]/g, (...m) => (m[0] === ')' ? '(' : ')')) // flip parentheses
        .split(' ')
        .reverse()
...
Renderer.getNames = function(nameStack, entry, opts) {
  ...
  if (entry.entries) {
    const nextDepth = entry.type === 'section' ? -1 : entry.type === 'entries' ? opts.depth + 1 : opts.depth
                ...
                case '@hit': {
                  return (
                    ...
                    (() => {
                      const n = Number(rollText)
                      if (isNaN(n)) {
                        throw new TypeError(`Could not parse "${rollText}" as a number!`)
                      }
                      return `${n >= 0 ? '+' : ''}${n}`
                    })()
                  )
